...
function loadExistingDimensions(): Record<string, Dimensions> {
...
function generate() {
  const existing = loadExistingDimensions();
  ...
  // Preserve video dimensions from previous runs when ffprobe is unavailable
  for (const [key, value] of Object.entries(existing)) {
    if (/\.(mp4|webm|mov)$/i.test(key) && !dimensionsMap[key]) {
